...
public class DeviceErrorProcessor
{
        ...
        string name)
    {
        ...
        var latestPerDevice = entries
            .GroupBy(e => e.DeviceName)
            .Select(g => g.OrderByDescending(e => e.DetectedAt).First());

        foreach (var entry in latestPerDevice)
        {

            var maxAgeMinutes = 3;
            var nowUtc = DateTime.UtcNow;
            if ((nowUtc - entry.DetectedAt).TotalMinutes > maxAgeMinutes)
                    continue;

            if ((nowUtc - entry.DetectedAt).TotalMinutes > maxAgeMinutes)
            {
                _logger.LogInformation($"Skipping old entry: {entry.DeviceName} at {entry.DetectedAt} (older than {maxAgeMinutes} minutes)");
                continue;
            }
